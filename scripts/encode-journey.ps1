param([Parameter(Mandatory=$true)][string]$state,[Parameter(Mandatory=$true)][string]$input)
...
& $ffmpeg -y -i $input -an -r 30 -pix_fmt yuv420p -c:v libvpx-vp9 -b:v 0 -crf 28 -g 60 "$outDir/webm/$state.webm"
