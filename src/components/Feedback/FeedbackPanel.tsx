...
export default function FeedbackPanel({open,onClose}:Props){
  ...
  return(<div style={wrap} role="dialog" aria-modal="true" aria-label="Feedback">
    <div style={card}>
      ...
      {status==='sent' && <div className="good" style={{marginBottom:8}}>Thank you â€” your feedback was sent.</div>}
      ...
      <div style={row}><textarea placeholder="Write your messageâ€¦" maxLength={MESSAGE_MAX} value={message} onChange={e=>setMessage(e.target.value)} style={{...input,minHeight:120}} /><div style={hint}>{message.length}/{MESSAGE_MAX}</div></div>
