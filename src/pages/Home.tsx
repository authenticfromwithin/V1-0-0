...
export default function Home(){
  const navigate=useNavigate(); const onReadyRef=useRef<MaybeFn|MaybeEl>(null);
  ...
  useEffect(()=>{ let mounted=true; (async()=>{ try{ const user=await auth.current?.(); if(mounted&&user) navigate('/quotes'); }catch{} })(); return ()=>{ mounted=false; }; },[navigate]);
